...
import { Priority, Project } from "@/app/types";
...
export const Form = ({
  ...
  createTaskAction: (formData: FormData, projectId: string) => Promise<void>;
}) => {
  const [priority, setPriority] = useState<Priority>("MEDIUM");

  // Хелпер для стилизации кнопки-триггера текущего приоритета
  ...
  return project ? (
    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 shadow-xl space-y-4">
      ...
      <form
        action={(formData) => createTaskAction(formData, project.id)}
    ...
    <p className="text-amber-400 bg-amber-950/30 border border-amber-900 p-4 rounded-lg">
      ⚠️ Проект не найден
    </p>
  );
};
